...
| UIコンポーネント | Vuetify | Material Designに基づくコンポーネント一式（テーブル・モーダル・フォーム部品など）を提供するライブラリ。実装速度を重視して採用 |
| 状態管理 | Pinia | Vue公式推奨の状態管理ライブラリ（Vuexの後継）。イベント一覧やフィルタ状態などをコンポーネント間で共有する |
| ルーティング | Vue Router | 一覧画面・カレンダー画面間のページ遷移を管理する |
| カレンダー表示 | Vuetify（`v-calendar`コンポーネント） | Vuetify付属のカレンダーコンポーネントを利用し、月表示・日付ごとのイベント表示・ステータス別の色分けを実装する |
...
[ブラウザ]
   ↓ HTTP（API通信 / JSON）
[Vue 3（TypeScript / Vite / Vuetify / Pinia / Vue Router）] ── フロントエンド
   ↓
[Rails（APIモード）] ── バックエンド
   ↓ Active Record
[MySQL]
